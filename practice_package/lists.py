...
def remove_invalid_emails(emails: list[str]) -> list[str]:
    return [email for email in emails if '@' in email and len(email) > 5 and email[0] != '@' and email[-1] != '@' and email.count('@') == 1]

def filter_palindromes(strings: list[str]) -> list[str]:
    return [string for string in strings if string.lower() == string[::-1].lower()]
